...
def make_signal(start_time, end_time, values):
    y=DiscreteSignal(start_time,end_time)
    for i in range(start_time,end_time+1):
        y.set_value_at_time(i,values[i-start_time])
    return y
...
def impulse_identity():
    y=DiscreteSignal(0,0)
    y.set_value_at_time(0,1)
    return y
...
def impulse_moving_average(length):
    y=DiscreteSignal(0,length-1)
    for i in range(length):
        y.set_value_at_time(i,(1/length))
    return y
...
def impulse_weighted_smoothing():
    y=DiscreteSignal(0,2)
    y.set_value_at_time(0,0.5)
    y.set_value_at_time(1,0.3)
    y.set_value_at_time(2,0.2)
    return y
...
def max_absolute_difference(first_signal, second_signal):
    val=0
    m=first_signal.times()
    n=second_signal.times()
    start=min(m[0],n[0])
    end=max(m[-1],n[-1])

    for i in range(start,end+1):
        val=max(val,abs(first_signal.get_value_at_time(i)-second_signal.get_value_at_time(i)))
