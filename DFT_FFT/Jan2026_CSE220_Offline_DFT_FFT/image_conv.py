...
def transform_2d(plane, engine):
    ...
    row_transformed=np.full((P,Q),0.0,dtype=np.complex128) 
    for i in range(P):
        row_transformed[i]=engine.transform(plane[i])

    res=np.full((P,Q),0.0,dtype=np.complex128)

    for j in range(Q):
        col = engine.transform(row_transformed[:, j])
        res[:, j] = col    
    return res
...
def inverse_2d(spectrum, engine):
    ...
    row_transformed=np.full((P,Q),0.0,dtype=np.complex128) 
    for i in range(P):
        row_transformed[i]=engine.inverse(spectrum[i])

    res=np.full((P,Q),0.0,dtype=np.complex128)
    for j in range(Q):
        col = engine.inverse(row_transformed[:, j])
        res[:, j] = col 
    return res
...
def convolve_plane(plane, kernel, engine, circular=False):
    ...
    if(circular==False):
        ...
        x=np.full((P,Q),0.0, dtype=np.float64)
        h=np.full((P,Q),0.0, dtype=np.float64)

        x[:H, :W] = plane

        h[:kh, :kw] = kernel

        x_transformed=transform_2d(x,engine)
        h_transformed=transform_2d(h,engine)

        conv_res=np.full((P,Q),0.0,dtype=np.complex128)
    
        conv_res = x_transformed * h_transformed
        conv_res = inverse_2d(conv_res, engine)
        conv_res = conv_res.real
        res = conv_res[kh//2:kh//2+H, kw//2:kw//2+W].copy()
        return res
        ...
        kernel_full = np.roll(kernel_full, shift=(-(kh//2), -(kw//2)), axis=(0,1))

        x_transformed=transform_2d(kernel_full,engine)
        h_transformed=transform_2d(plane,engine)
        ...
        return conv_res.astype(np.float64)
...
def convolve_image(image, kernel, engine, circular=False):
    ...
    H=image.shape[0]
    W=image.shape[1]
    res=np.full((H,W,3),0.0,dtype=np.complex128)
    for i in range(0,3):
        res[:, :, i] = convolve_plane(image[:, :, i], kernel, engine, circular)
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    if engine.name == "fft":
        t_h, t_w = next_power_of_two(lin_h), next_power_of_two(lin_w)
    else:
        t_h, t_w = lin_h, lin_w
    ...
    max_diff = np.max(np.abs(spectral - direct))
    verification = "MATCH" if max_diff < 1e-9 else "MISMATCH"

    mode = "RGB" if image.ndim == 3 else "gray"
    lines = [
        "Task B -- 2D convolution through the frequency domain",
        "%-20s: %s  (%d x %d, %s)" % ("image", path, W, H, mode),
        "%-20s: %s  (%d x %d)" % ("kernel", kernel_name, kh, kw),
        "%-20s: %s" % ("engine", engine_name),
        "%-20s: %d x %d" % ("linear-conv size", lin_h, lin_w),
        "%-20s: %d x %d" % ("transform size", t_h, t_w),
        "max |spectral - direct| on 64x64 crop : %.3e" % max_diff,
        "%-20s: %s" % ("verification", verification),
    ]
