...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    for i in range(len(limbs)-2,-1,-1):
        st+=str(limbs[i]).zfill(base_digits)
...
def multiply_transform(a, b, engine):
    ...
    a2=np.full(N,0,dtype=np.int64)
    b2=np.full(N,0,dtype=np.int64)
    ...
    a_transformed=engine.transform(a2)
    b_transformed=engine.transform(b2)

    res=np.full(N,0,dtype=np.complex128)
    for i in range(N):
        res[i]=a_transformed[i]*b_transformed[i]

    res=engine.inverse(res)
    res=np.round(res.real).astype(np.int64)
    return res, N
...
def multiply_schoolbook(a, b):
    ...
    """
    # N=len(a)+len(b)-1
    # res=np.full(N,0,dtype=np.int64)
    ...
    for k in range(N):
        i0 = max(0, k - lb + 1)
        i1 = min(la - 1, k)
        res[k] = np.dot(a[i0:i1+1], b_rev[lb-1-k+i0 : lb-1-k+i1+1])
    return res, N
...
def multiply(text_a, text_b, method):
    ...
    if method=="schoolbook":
        res,N=multiply_schoolbook(limbs_a,limbs_b)
    else:
        res,N=multiply_transform(limbs_a,limbs_b,engine)
   
    st=from_limbs(sign,res)

    return (st,N,limbs_a,limbs_b)
...
def run_single(path, method, out_dir):
    ...
    """
    a,b=read_operands(path)
    st,N,limbs_a,limbs_b=multiply(a,b,method)
    direct=int(a)*int(b)
    ...
    lines=[]
    lines.append("Task A -- big-integer multiplication by spectral convolution")
    lines.append(f"input file          : {path}")
    lines.append(f"method              : {method}")
    lines.append(f"digits of A / B     : {lena} / {lenb}")
    lines.append(f"base                : 10^{BASE_DIGITS}")
    lines.append(f"limbs of A / B      : {len(limbs_a)} / {len(limbs_b)}")
    lines.append(f"transform length N  : {N}")
    lines.append(f"digits of product   : {length}")
    lines.append(f"verification        : {match}")
    write_text(f"{out_dir}/product.txt",st)
    write_report(f"{out_dir}/report.txt",lines)
