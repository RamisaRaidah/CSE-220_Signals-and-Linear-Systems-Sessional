...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        sin_a=np.sin(2*np.pi*np.outer(self.x,self.u))
        sin_b=np.sin(2*np.pi*np.outer(self.y,self.v))
        cos_a=np.cos(2*np.pi*np.outer(self.x,self.u))
        cos_b=np.cos(2*np.pi*np.outer(self.y,self.v))
        

        real1=np.trapezoid(self.I[:,:,None]*cos_a[None,:,:],self.x,axis=1)
        real1=np.trapezoid(real1[:,None,:]*cos_b[:,:,None], self.y,axis=0)

        real2=np.trapezoid(self.I[:,:,None]*sin_a[None,:,:],self.x,axis=1)
        real2=np.trapezoid(real2[:,None,:]*sin_b[:,:,None], self.y,axis=0)

        real=real1-real2

        imag1=np.trapezoid(self.I[:,:,None]*sin_a[None,:,:],self.x,axis=1)
        imag1=np.trapezoid(imag1[:,None,:]*cos_b[:,:,None], self.y,axis=0)

        imag2=np.trapezoid(self.I[:,:,None]*cos_a[None,:,:],self.x,axis=1)
        imag2=np.trapezoid(imag2[:,None,:]*sin_b[:,:,None], self.y,axis=0)
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        I1=np.trapezoid(p[:,None,:]*cos_a[None,:,:],self.u,axis=2)
        I2=np.trapezoid(q[:,None,:]*sin_a[None,:,:],self.u,axis=2)
